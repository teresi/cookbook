\section[Demerara Syrup]{Demerara Simple or Rich Syrup}
\ragmarpar{Mike likes Piloncillo for it's complex flavor.}

\begin{recipestats}[
        servings=$1~C$,
        preptime=5~\minute,
        source=Mike \& Jane,
        original=\citefield{cocktailSeminars2021}{title}~\cite{cocktailSeminars2021},
    ]
\end{recipestats}

\begin{recipeabstract}
    This syrup is has a richer flavor compared to \nameref{simpleSyrup}.
    The measurements below use the traditional 1:1 and 2:1 ratio by volume.
    Demerara, Piloncillo, and Turbinado are raw granulated sugars, or \gls{NCS},
    meaning it has been reduced with evaporation rather than a
    centrifuge\cite{worldSugarGuide}.
\end{recipeabstract}
\ragmarpar{Demerara, Turbinado, \& Piloncillo are $~0.81~g/ml$ or $192~g/C$
\cite{aquaCalcDemerara} \cite{aquaCalcTurbinado} \cite{aquaCalcPiloncillo}.}

\begin{ingredientcolumns}
    \begin{ingredientblock}[simple]
        \ingredient[1][C]{water}\\
        \ingredient[\onehalf][C]{sugar}\\
        \ingredient[\onehalf][C]{Demerara sugar}
    \end{ingredientblock}

    \begin{ingredientblock}[rich]
        \ingredient[\onehalf][C]{water}\\
        \ingredient[\onehalf][C]{sugar}\\
        \ingredient[\onehalf][C]{Demerara sugar}
    \end{ingredientblock}
\end{ingredientcolumns}

\smallskip
\begin{preparation}
\item Boil water in a kettle, measure, add to a small saucepan.
\item Add the sugars, turn heat to meadium, stir until dissolved,
\item Let cool, refridgerate up to $3~weeks$.
\end{preparation}

\recipeend%
