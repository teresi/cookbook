...
% [ ] add a vegan / vegetarian marking (\faLeaf ?) on the recipe name? or in the stats?
% [ ] update minutes / min to use commands, standardize to abbreviation? (min / hr)
%     this will be good to reduce column size in recipe stats entries
% [ ] add fan time / vegan to recitpe stats (optionall)
%     prep   electric  fan
...
inner=0.75in,%  3/4" inner looks good
outer=1.25in,%
papersize={6in,9in},%
...
\renewcommand{\bottomtitlespace}{0.12\textheight}  % default 0.2\textheight
...
\keys_define:nn { mymodule/recipestats }
{
	...
	convectiontime .tl_set:N = \l_mymodule_convectiontime_tl,
	vegetarian .tl_set:N = \l_mymodule_vegetarian_tl,
}
\NewDocumentEnvironment{recipestats}{O{}}
{
	...
	\begin{tabular}{clclcl}
		...
		\tl_if_empty:NTF \l_mymodule_convectiontime_tl
		{& \\}
		{\Fanoven & \l_mymodule_convectiontime_tl \\}
		...
		\tl_if_empty:NTF \l_mymodule_vegetarian_tl
		{& \\}
		{\faLeaf & \l_mymodule_vegetarian_tl \\}
}
{
	\end{tabular}
...
%	\begin{samepage}
%	\textbf{VARIATIONS}%
	\paragraph{VARIATIONS}  % TODO use upper case font?
...
\newcommand{\recipeend}[0]{
	{\centering \pgfornament[scale=0.55]{88} \smallbreak}
}
...
%\def\vegan#1{\marginpar[\sloppy\RaggedLeft\hspace{0pt}#1]{\sloppy\RaggedRight\hspace{0pt}#1}}
...
\input{./pickles/quick_pickles}
{
	\let\section\subsection
	\let\subsection\subsubsection
	\input{./pickles/red_onion}
	\input{./pickles/jalepenos}
	\input{./pickles/radishes}
}
...
\minitoc
\clearpage
\input{./desserts/ItalianTricolor}
